...
// 카드 전체 조회 -> = 생성 
async function cardGetAllApi(success, fail) {
    await local.get(`/plan/card`).then(success).catch(fail);
}
...
async function cardDeleteApi(cardId, success, fail) {
    await local.delete(`/plan/card/delete/${cardId}`).then(success).catch(fail);
}
